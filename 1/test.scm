...
(define (n^p_mod_p n p)  ;n,p -> n^p \mod p
  (remainder (expt n p) p))
(define (fermat-pass? n p)
  (= (n^p_mod_p n p) n))
...
(define (greatest-smaller-prime p)
  (cond ((= p 0) false)
        ((fermat-test? p) p)
        (else (greatest-smaller-prime (- p 1)))))
(define (least-bigger-prime p)
  (cond ((eq? p 0) 2)
        ((fermat-test? p) p)
        (else (least-bigger-prime (+ p 1)))))

(define (rand-in-interval x factor lower upper)
  (if (< x 0)
    (- 0 (rand-in-interval (abs x) factor (abs upper) (abs lower)))
    (modulo (+ (* factor x) (least-bigger-prime lower)) (greatest-smaller-prime upper))))

(define (make-numgen lower upper)
  (let ((rand_init 1))
    (define (update)
      (set! rand_init
          (rand-in-interval rand_init
                            (least-bigger-prime (ceiling (/ (abs (- upper lower)) 100)))
                            lower
                            upper)))
    update))
...
(define symbol-numgen
  (make-numgen 0 30)) ;greater smaller prime for 30 is 29
(define (symbolgen)
  (num->symbol (symbol-numgen)))

;to set up all other generators, now, is simply a matter of finding the right bijections... 

;then generate-predicate could return something like (lambda (x) (eq? x (generate-symbol))
;                                                 or (lambda (x) (< x (generate-number)))
