...
;;Exercise 4.24:
;;Did this exercise in another file -- test.scm
...
;;Each argument is evaluated before the body of unless is entered. In particular, (factorial (- n 1)), and this is true irrespective of which n we use. Thus, we get an infinite sequence of
;;function calls.
;;In a normal order language, the body is entered each time, and so in particular the if statement guides the function calls based on whether or not (= n 1). When it does, 1 is returned, and then
;;the function before it returns that 1 and it is multiplied by the 2 and return to the one before it, and so on, until 5*4*3*2*1=5! is returned, as desired. So, yeah, in normal order evaluation
;;it goes as planned.

;;Exercise 4.26:
;;to implement unless as a special form, we first need selectors:
...
(define (eval-assignment-lazy exp env)
  (set-variable-value! (assignment-variable exp)
...
;;Well, first off, let's consider what happens when we define w:
;;(eval-lazy '(define w (id (id 10))) env)
;;=(eval-definition-lazy '(define w (id (id 10))) env)
;;=(define-variable! w
;;                  (eval-lazy (id (id 10)) env)
;;                  env)
;;which leads us to looking at:
;;(eval-lazy (id (id 10)) env)
;;=(apply-lazy (actual-value 'id env)
;;             ((id 10))
;;             env)
;;=(apply-lazy (procedure (x) ((set! count (+ count 1)) x) env)
;;             ((id 10))
;;             env)
;;=(eval-sequence-lazy
;;   ((set! count (+ count 1)) x)
...
;;   ((set! count (+ count 1)) x)
;;   *env)
;;= ('thunk '(id 10) env), and count=1.
;;where *env= (extend-environment
;;              (x)
;;              (('thunk '(id 10) env))
;;              env)
;;, and so w is  bound to ('thunk '(id 10) env).
;;Thus we get:
;; L-Eval input:
;;count
;; L-Eval value:
...
;;w
;;=(actual-value 'w env)
;;=(force-it (eval-lazy 'w env))
;;=(force-it ('thunk '(id 10) env))
;;=(actual-value '(id 10) env)
;;=(force-it (eval-lazy '(id 10) env))
...
;;     (('thunk 10 env))
;;     env))
;;=('thunk 10 env), and count=2.

;; so
;;=(force-it (eval-lazy '(id 10) env))
;;=(force-it (thunk 10 env))
;;=(actual-value 10 env)
;;=10.
...
;;(eval-lazy '(proc q) genv)
;;=(apply-lazy 'proc (q) genv)
;;=(eval-sequence (procedure-body (lookup-variable-value 'proc genv))
;;                (extend-environment (procedure-parameters (lookup-variable-value 'proc genv))
;;                                    (list 'thunk q genv)
;;                                    (procedure-environment (lookup-variable-value 'proc genv))))
